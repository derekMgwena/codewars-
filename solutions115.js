...
function toRange(arr) {
  const [head, ...tail] = [...arr].sort((a, b) => a - b);
  return tail
    .reduce((s, n) =>
        (([a, b]) => s.concat(n <= b + 1 ? [[a, n]] : [[a, b], [n, n]]))(s.pop()),
      [[head, head]])
    .map(([a, b]) => a === b ? a : `${a}_${b}`).join();
}

function toArray(str) {
  return str
    ? [].concat(...str.split(",").map(s =>
      (([a, b = a]) => Array(b - a + 1).fill().map(() => a++))(s.split("_"))))
    : [];
}